...
JAVA_CLASS_DIR="$(realpath "$JAVA_CLASS_DIR")"

javac -cp "$JAVA_SRC_DIR:$ROOTDIR/res/*" -d "$BUILD_DIR/java/" "$JAVA_SRC_DIR/eu/lixko/csgointernals/Main.java"
g++ -fPIC -shared -Wl,--no-as-needed,-rpath,"$JVM_ROOT/lib/server/" -I"$JVM_ROOT/include/" -I"$JVM_ROOT/include/linux/" -L"$JVM_ROOT/lib/server/" -DJAVA_CP="\"$ROOTDIR/res/:$JAVA_RES_JARS:$JAVA_CLASS_DIR\"" -lpthread -ldl -ljvm -o "$BUILD_DIR/libjvmloader.so" "$ROOTDIR/jvm_loader.cpp"